...
class GA_base_NN:
    ...
    def __init__(self, bounds, fitnessFunction, distribution_comparison, populationSize=10, mutationRate=0.03,
                 ...
                 ui_plot=True, read_from_file=False, read_from_file_name="sim_val1"):
        # GA parameters
    ...
    def run(self):
        ...
        # best parameters
        print('best parameters: {}'.format(self.best.compute_output(self.input)))

        # start new population
        if self.islands_model == False:
            self.reproduction_steps()
        if not self.islands_model:
            self.computeFitness(0, plot=True)
        else:
            parameters = self.best.compute_output(self.input)
    ...
    def reproduction_steps(self):
        ...
        print('current number 1: {}'.format(self.newPopulation[0]))
    ...
    def computeFitness(self, i, repeat=1, plot=False):
        ff = list()
    ...
    def reproduce_rank(self):
        ...
        for i in range(len(rank)):
            probabilities.append(i+1 / np.sum([x for x in range(len(rank))]))

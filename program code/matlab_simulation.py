...
#             binEdges = [x for x in (range(np.min(d) - 0.5), np.max(d) + 0.5, step)]  # [min(d) - 0.5:step: max(d) + 0.5]
...
#             if isempty(coupled)
#             bar(midbins, [dead_counts;
#             living_counts]','
#             stacked
#             ')
#             legend({'Dead', 'Living'});
#             else
#             bar(midbins, [coupled_counts;
#             dead_counts;
#             living_counts]','
#             stacked
#             ')
...
#             ylabel('Frequency')
#             title(
#                 strcat(' conversion=', num2str(conversion), ' time=', num2str(time), ' PDI=', num2str(PDI), ' DPn=',
#                        num2str(DPn), ' DPw=', num2str(DPw)))
#             end
#
...
#             living( and (
#                 r < (p_growth * monomer_ratio + (p_death * monomer_ratio)), r >= p_growth * monomer_ratio))=1;
#             number_killed = size(living( and (
#                 r < (p_growth * monomer_ratio + (p_death * monomer_ratio)), r >= p_growth * monomer_ratio)), 2);
...
#             # Bryn
#             p_success=p_dead_react / (living(which_living_attacked) ^ max(1- living(which_living_attacked) * (1-l_exponent) / l_naked, l_exponent) * dead(dead_counter) ^ (1- dead(dead_counter) * (1-l_exponent) / l_naked, l_exponent));
#             # If our random number is less than this probability then
#             # add the dead length to the living and remove the dead from
#             # the dead pool
...
#             if monomer_pool > 0:
#                 if
#             sum(which_polymers_killed) > monomer_pool: \
#                 monomer_pool = 0;
#             else:
